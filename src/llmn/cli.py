...
from llmn.config import get_settings
from llmn.db import get_db
from llmn.db.queries import (
    FoodQueries,
...
app = typer.Typer(
    help="LLM Nutrition - diet optimization for humans and AI agents",
...
def require_food_data() -> None:
    ...
    if not has_foods:
        ...
        console.print("Run: [cyan]llmn init <path-to-usda-csv>[/cyan]")
        raise typer.Exit(1)
...
def init(
    ...
    from llmn.data.usda_loader import USDALoader
        ...
        if not food:
            if json_output:
                output_json({
                    ...
                    "suggestions": ["Use 'llmn search <query>' to find valid FDC IDs"],
                })
            else:
...
def optimize(
    ...
    from llmn.agent.response import create_response
    from llmn.export.formatters import format_result
    from llmn.explore.diagnosis import diagnose_infeasibility
    from llmn.optimizer.constraints import (
        deserialize_profile_json,
        has_meals_section,
        load_multiperiod_profile_from_yaml,
        load_profile_from_yaml,
    )
    from llmn.optimizer.models import OptimizationRequest, NutrientConstraint
        ...
        if not profile_file.exists():
            if json_output:
                output_json({
                    ...
                    "suggestions": [
                        "Check the file path",
                        "Use 'llmn profile list' to see saved profiles",
                    ],
                })
            else:
            ...
            if not profile_row:
                if json_output:
                    output_json({
                        ...
                        "suggestions": [
                            "Use 'llmn profile list' to see available profiles",
                            "Use 'llmn profile create' to create a new profile",
                        ],
                    })
                else:
    ...
    with db.get_connection() as conn:
        ...
        if pattern and not foods:
            from llmn.data.pattern_pools import build_food_pool
                ...
                if not pattern_food_ids:
                    if json_output:
                        output_json({
                            ...
                            "suggestions": ["Check pattern names with 'llmn patterns list'"],
                        })
                    else:
        ...
        if template:
            ...
            from llmn.templates.models import SelectionStrategy
        ...
        if use_multiperiod and mp_request is not None:
            ...
            if practical and mp_result.success:
                from llmn.export.meal_consolidator import consolidate_result, consolidated_result_to_dict
                from llmn.export.shopping_list import generate_shopping_list, shopping_list_to_dict
            ...
            if json_output:
                ...
                if mp_result.success:
                    if consolidated:
                        ...
                        from llmn.export.meal_consolidator import format_consolidated_result
                        from llmn.export.shopping_list import format_shopping_list
        ...
        if not json_output:
            ...
            if request.include_tags and total_foods == 0:
                ...
                console.print(
                    "[yellow]Use 'llmn tags add <fdc_id> <tag>' to tag foods first.[/yellow]"
                )
        ...
        if max_foods_in_solution is not None:
            from llmn.optimizer.sparse_solver import solve_sparse_diet
    ...
    if save:
        from llmn.export.formatters import JSONFormatter
    ...
    if json_output:
        ...
        if result.success:
            ...
            if result.total_cost and result.total_cost > 15:
                suggestions.append(
                    "Daily cost is high - try 'llmn optimize --minimize-cost' or expand food pool"
                )
    ...
    if verbose and result.success and result.kkt_analysis:
        from llmn.export.formatters import KKTFormatter
    ...
    if allocate_meals and result.success and result.foods:
        from llmn.export.meal_allocator import allocate_to_meals, format_meal_allocation_text
...
def optimize_batch_cmd(
    ...
    from llmn.optimizer.batch import (
...
def export_for_llm(
    ...
    from llmn.export.llm_prompt import LLMPromptGenerator
    from llmn.optimizer.models import FoodResult, NutrientResult, OptimizationResult
...
def schema_constraints() -> None:
    ...
    from llmn.agent.schema import get_constraint_schema
...
def schema_nutrients() -> None:
    ...
    from llmn.agent.schema import get_nutrient_list
...
def schema_tags() -> None:
    ...
    from llmn.agent.schema import get_tag_list
    db = get_db()
    with db.get_connection() as conn:
...
def schema_foods() -> None:
    ...
    from llmn.agent.schema import get_food_filter_schema
...
def schema_all() -> None:
    ...
    from llmn.agent.schema import get_full_schema
    db = get_db()
    with db.get_connection() as conn:
...
def explore_foods_cmd(
    ...
    from llmn.explore.foods import FoodFilter, explore_foods
...
def explore_high_nutrient(
    ...
    from llmn.explore.foods import find_high_nutrient_foods
    from llmn.data.nutrient_ids import NUTRIENT_IDS, NUTRIENT_UNITS
...
def explore_compare(
    ...
    from llmn.explore.foods import compare_foods
...
def explore_suggest_pools(
    ...
    Examples:
        llmn explore suggest-pools --json
        llmn explore suggest-pools --strategies balanced,high_protein --tag staple
    """
    from llmn.explore.suggest import format_pool_suggestions, suggest_pools
...
def explore_runs(
    ...
    Examples:
        llmn explore runs
        llmn explore runs --limit 5 --profile cutting
    """
    from llmn.explore.compare import format_run_list, list_runs
...
def explore_compare_runs(
    ...
    Examples:
        llmn explore compare-runs 5 8
        llmn explore compare-runs 5 8 --json
    """
    from llmn.explore.compare import compare_runs, format_run_comparison
...
def prices_import(
    ...
    from llmn.data.price_loader import PriceLoader
...
def profile_create(
    ...
    from llmn.optimizer.constraints import load_profile_from_yaml
...
def patterns_list(
    ...
    from llmn.data.dietary_patterns import list_patterns, DIETARY_PATTERNS
    ...
    if json_output:
        ...
        console.print(table)
        console.print(f"\n[dim]Use with: llmn optimize --pattern <name>[/dim]")
...
def patterns_info(
    ...
    from llmn.data.dietary_patterns import get_pattern
    ...
    if not pattern:
        if json_output:
            output_json({
                ...
                "suggestions": ["Use 'llmn patterns list' to see available patterns"],
            })
        else:
...
def user_create(
    ...
    from llmn.tracking.models import UserProfile
    ...
    if profile is None:
        if json_output:
            output_json({
                ...
                "suggestions": ["Create a profile with: llmn user create --age 35 --sex male --height 70 --weight 180"],
            })
        else:
            console.print("[red]No user profile found[/red]")
            console.print("Create one with: llmn user create --age 35 --sex male --height 70 --weight 180")
        raise typer.Exit(1)
...
def weight_add(
    ...
    from llmn.tracking.queries import UserQueries, WeightQueries
        ...
        if profile is None:
            if json_output:
                output_json({
                    ...
                    "command": "weight add",
                    "errors": ["No user profile found"],
                    "suggestions": ["Create a profile first: llmn user create ..."],
                })
            else:
...
def weight_trend(
    ...
    from llmn.tracking.diagnostics import format_weight_report, generate_weight_report
    from llmn.tracking.queries import UserQueries
...
def calories_log(
    ...
    from llmn.tracking.queries import CalorieQueries, UserQueries
...
def tdee_estimate(
    ...
    from llmn.tracking.diagnostics import (
        calculate_mifflin_tdee,
        calculate_mifflin_tdee_at_weight,
    )
    from llmn.tracking.models import TDEEEstimate
        ...
        if len(weight_history) < 7:
            if json_output:
                output_json({
                    ...
                    "suggestions": ["Log your weight daily with: llmn weight add <weight>"],
                })
            else:
    ...
    if json_output:
        ...
        console.print()
        console.print("[dim]Run 'llmn tdee progress' for full report[/dim]")
...
def tdee_targets(
    ...
    from llmn.tracking.diagnostics import calculate_mifflin_tdee, generate_weight_report
    from llmn.tracking.queries import CalorieQueries, TDEEQueries, UserQueries
    ...
    if report is None:
        if json_output:
            output_json({
                ...
                "suggestions": ["Log at least 2 weight entries with: llmn weight add <weight>"],
            })
        else:
            console.print("[yellow]Not enough weight data for progress report[/yellow]")
            console.print("Log weight with: llmn weight add <weight>")
        raise typer.Exit(1)
